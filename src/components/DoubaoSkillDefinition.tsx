...
3. 【3:4 封面设计与短标题（全自动垫图 · 原片人物表情 · 双轨短标题）】：
   ...
   - 【红线审查与封面生成二分机制（核心原则）】：
     * 不触碰审核红线：绝不重新生图！直接使用视频自动截图作为封面底图，并在适宜留白处打上大字短标题直出，100%还原生活实况质感；
     * 只有触碰平台审核红线（检测到打斗斗殴、流血、危险管制道具或暴力敏感画面）：才启动以该截图为底图，以图生图重新生成【3D仿真人（类似Pixar 3D高质感CG，彻底避免真人暴力血腥被封禁）】！
   - 若触碰红线调用豆包以图生图生成 3:4 比例 3D 仿真人海报，并在画面适宜留白处大字打上全中文短标题！
...
- 【红线审查与封面生成二分机制（核心逻辑）】：
  1. 不触碰平台审核红线（常规实况视频）：绝不重新生成图片！直接使用视频自动截图作为底图，并在适宜留白处打上大字短标题即可直出（100%还原真实实况质感，零生图消耗，秒级直出）；
  2. 只有触碰平台审核红线（检测到打斗斗殴、流血、危险管制道具或暴力冲突画面，直接发实况截图会被平台限流或封禁）时：才启动以该截图为垫图底图，以图生图重新生成【3D仿真人（3D CG动画质感）】进行降敏重绘，以此规避平台审查封禁！
- 同时输出中英文双语 Prompt。
