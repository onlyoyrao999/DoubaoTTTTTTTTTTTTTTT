...
3. 3:4 冲击力《真实写实封面》方案（Cover）：
   - 核心原则：主要以读取视频内容，截取一张最适合做封面的高潮分镜图片进行文字封面设计加工。
   - 【极端重要：全流程自动化垫图 + 面部表情不用夸张，注重真实还原，保留原始表情做写实封面】：
   ...
   - 【极端重要：红线审查二分机制（触碰红线才重新以图生图3D仿真人，不触碰红线直接截图打字直出）】：
     1) 【不触碰红线（常规日常实况）】：绝不重新生图！直接使用抽取的视频高潮分镜截图作为封面底图，压上醒目大字短标题即可秒级直出，100%忠实原片生活实况；
     2) 【触碰红线（含打斗、流血撕扯、暴力凶器或平台敏感违规内容）】：由于实拍图直接发布必遭平台限流或封禁，封面必须以该截图为底图，启动【以图生图生成3D仿真人（Pixar/CGI 风格，通过 3D 动画材质彻底规避真人暴力审核）】！
...
3. 【3:4 真实写实视觉封面】：
   ...
   - 人物形象要求：纯真写实纪实画风，面部表情不用刻意夸张，注重还原很原始的生活实况神态与自然微表情。
